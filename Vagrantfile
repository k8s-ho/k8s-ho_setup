...
Vagrant.configure("2") do |config|
    config.vm.define "master-k8sHo" do |cfg|
      ...
      cfg.vm.provision "shell", path: "k8s_env.sh", args: NodeCount
      cfg.vm.provision "shell", path: "k8s_package.sh", args: k8s_ver 
  ...
  (1..NodeCount).each do |i|
    config.vm.define "worker_#{i}-k8sHo" do |cfg|
      ...
      cfg.vm.synced_folder "../data", "/setup", disabled: true
      cfg.vm.provision "shell", path: "k8s_env.sh", args: NodeCount
      cfg.vm.provision "shell", path: "k8s_packge.sh", args: k8s_ver
